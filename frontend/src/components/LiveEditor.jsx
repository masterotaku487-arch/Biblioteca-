...
// ===================================================================
const LIVE_SERVER_PORT = 8001;
const getLiveServerUrl = () => {
  const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
  const hostname = new URL(API).hostname;
  return `${protocol}//${hostname}:${LIVE_SERVER_PORT}`;
};
...
const LiveEditor = ({ file, team, user, onClose }) => {
  ...
  const loadFileContent = async () => {
    try {
      ...
      if (response.data.type === 'text') {
        ...
        const msg = `[Arquivo binário - ${file.file_type}]\nTamanho: ${(file.file_size / 1024).toFixed(2)} KB\n\nEdição não suportada no modo Live.`;
        setContent(msg);
  ...
  const connectWebSocket = () => {
    ...
    const wsUrl = `${liveServerUrl}/ws/live/${team.id}/${file.id}`;

    console.log('🔌 Conectando ao Live Server:', wsUrl);
    ...
    try {
      ...
      wsRef.current.onopen = () => {
        ...
        console.log('📤 Enviando join:', joinMessage);
        wsRef.current.send(JSON.stringify(joinMessage));
        
        toast.success('Conectado ao Live Editor!');
      };
      ...
      wsRef.current.onclose = (event) => {
        ...
        if (reconnectAttempts.current < maxReconnectAttempts) {
          ...
          console.log(`🔄 Reconectando em ${delay}ms (${reconnectAttempts.current}/${maxReconnectAttempts})...`);
          
          reconnectTimeoutRef.current = setTimeout(() => {
            connectWebSocket();
          }, delay);
        } else {
          toast.error('Não foi possível conectar ao Live Editor');
        }
      };
    } catch (error) {
  ...
  const handleWebSocketMessage = (data) => {
    switch (data.type) {
      case 'user_joined':
        ...
        toast.info(`${data.username} entrou`, { duration: 2000 });
        break;
        
      case 'user_left':
        setActiveUsers(prev => prev.filter(u => u.username !== data.username));
        toast.info(`${data.username} saiu`, { duration: 2000 });
        break;
      ...
      case 'file_saved':
        ...
        setUnsavedChanges(false);
        if (data.username !== user.username) {
          toast.success(`${data.username} salvou o arquivo`);
        }
        break;
        
      case 'error':
        toast.error(data.message || 'Erro no Live Editor');
        break;
        
      default:
        console.log('⚠️ Mensagem desconhecida:', data.type);
    }
  };
      ...
      <div className="w-72 bg-white border-l flex flex-col">
        ...
        <div className="p-4 border-t bg-gray-50">
          <div className="space-y-2 text-xs text-gray-600">
            ...
            <div className="flex justify-between">
              <span>Servidor:</span>
              <span className="font-medium text-green-600">Live :8001</span>
            </div>
          </div>
        </div>
      </div>
