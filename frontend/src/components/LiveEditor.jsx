...
const LiveEditor = ({ file, team, user, onClose }) => {
  ...
  const connectWebSocket = () => {
    ...
    try {
      // Criar WebSocket SEM segundo parâmetro (isso causa o erro isTrusted)
      ...
      wsRef.current.onclose = (event) => {
        ...
        if (reconnectAttempts.current < 5) {
          ...
          console.log(`🔄 Tentando reconectar (${reconnectAttempts.current}/5) em ${delay}ms...`);
          
          reconnectTimeoutRef.current = setTimeout(() => {
            connectWebSocket();
          }, delay);
        } else {
          toast.error('Não foi possível conectar ao Live Editor. Tente novamente mais tarde.');
        }
      };
    } catch (error) {
