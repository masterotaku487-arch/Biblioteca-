...
const TeamsPanel = ({ user, onTeamsUpdate }) => {
  ...
  const connectWebSocket = () => {
    const ws = new WebSocket(`ws://localhost:8000/api/ws/canvas/${selectedTeam.id}`);
  ...
  const handleDeleteTeam = async (teamId) => {
    if (!window.confirm("Deletar este time? Todos os arquivos do time serão removidos.")) return;
    try {
  ...
  }, [elements, currentElement, users, user.username]);
